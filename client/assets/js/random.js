...
function displayPage(page) {
  ...
  pages[page].forEach((anime) => {
    document.getElementById("listOfRandomAnime").innerHTML +=
      ...
      '<br>Link <a href="/anime/' +
      anime.nameNoSpace +
      '">http://animeplanet.tv:85/anime' +
      anime.nameNoSpace +
      "</a>" +
      "</span>" +
      "</div>" +
...
function bubbleSortAlgo(arraaytest) {
  ...
  for (i = 0; i < len; i++) {
    for (j = 0, stop = len - i; j < stop; j++) {
      if (
        parseFloat(arraaytest[j].score) > parseFloat(arraaytest[j + 1].score)
      ) {
...
function GenList() {
  ...
  for (
    let i = 0;
    i < list.length / parseInt(document.getElementById("AmmountPerPage").value);
    i++
  ) {
    pages.push([]);
    for (
      let x = 0;
      x < parseInt(document.getElementById("AmmountPerPage").value);
      x++
    ) {
      pages[i].push(
        list[x + i * parseInt(document.getElementById("AmmountPerPage").value)],
      );
    }
  }
