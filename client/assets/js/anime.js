...
function epChange(ep) {
  ...
  if (anime.ep.length > ep + 1) {
    document.getElementById("nextEp").onclick = function () {
      epChange(ep + 1);
    };
  ...
  if (ep > 0) {
    document.getElementById("previousEp").onclick = function () {
      epChange(ep - 1);
    };
  ...
  for (let i = 0; i < EpisodesSeen.length; i++) {
    if (ep == EpisodesSeen[i]) alreadySeen = true;
  }
...
socket.on("AnimeData", function (data) {
  ...
  if (EpisodesSeen == undefined) EpisodesSeen = [];
  ...
  document.getElementById("ep-" + EpisodesSeen[0]).style.backgroundColor =
    "#061033";

  for (let i = 0; i < EpisodesSeen.length; i++) {
    document.getElementById("ep-" + EpisodesSeen[i]).style.backgroundColor =
      "#061033";
  }
});
