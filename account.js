    ...
    checkKey: function (key) {
        for (let i = 0; i < credentials.length; i++) {
            for (let x = 0; x < credentials[i].keys.length; x++) {
                if (key == credentials[i].keys[x].key && credentials[i].keys[x].keyExpo > Date.now()) {
                    if(credentials[i].keys[x].rememberMe)
                        credentials[i].keys[x].keyExpo = Date.now() + (expoTime * 90);
                    else 
                        credentials[i].keys[x].keyExpo = Date.now() + expoTime;
    ...
    SetupSocket: function (socket) {
        socket.on("login", function (data) {
            ...
            for (var i = 0; i < credentials.length; i++) {
                if (credentials[i].username == data.username && credentials[i].passwd == data.passwd) {
            ...
            if (credentialsTrue) {
                ...
                var expo;
                if(data.rememberMe)
                    expo = (Date.now() + (expoTime*90))
                 else
                    expo = Date.now() + expoTime
        ...
        socket.on('keyCheck', function (key) {
            if (module.exports.checkKey(key) != null) {
                ...
                logFile("Key Check IP:" + socket.handshake.address,{ip:socket.handshake.address,key:key})
            }
        });
